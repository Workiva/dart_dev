...
void main() {
  ...
  group('buildArgs', () {
    test('defaults to analyze command', () {
      expect(buildArgs(), orderedEquals(['analyze']));
    });
  ...
  group('logCommand', () {
    test('with <=5 entrypoints', () {
      expect(
        ...
        emitsThrough(infoLogOf(contains('dart analyze -t a b c d e'))),
      );
      logCommand(['analyze', '-t'], ['a', 'b', 'c', 'd', 'e']);
    });

    test('with >5 entrypoints', () {
      expect(
        Logger.root.onRecord,
        emitsThrough(infoLogOf(contains('dart analyze -t <6 paths>'))),
      );
      logCommand(['analyze', '-t'], ['a', 'b', 'c', 'd', 'e', 'f']);
    });

    test('with >5 entrypoints in verbose mode', () {
      expect(
        Logger.root.onRecord,
        emitsThrough(infoLogOf(contains('dart analyze -t a b c d e f'))),
      );
      logCommand(
        ['analyze', '-t'],
        ['a', 'b', 'c', 'd', 'e', 'f'],
